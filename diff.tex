...
%DIF DEL old.tex           Sun Jun 21 00:26:06 2026
%DIF ADD EinsteinTDE.tex   Tue Jul 14 16:55:05 2026
%DIF < \documentclass[final,12pt]{article}
%DIF -------
\documentclass[submitted,12pt]{article} %DIF > 
%DIF -------
...
\begin{abstract}
As early as 1907, Einstein realized that the second-order transverse Doppler redshift reported by Stark in the spectral lines of fast-moving ions in canal rays could serve as a direct experimental test of time dilation. He rejected Stark’s \emph{dynamical explanation} of the effect as a real frequency change of the moving ions. Instead, assuming that ions of the same kind emit the same spectral lines when at relative rest, Einstein offered a \emph{kinematical explanation} of the frequency shift as a consequence of time dilation. He labeled the effect \textit{apparent}, since \DIFdelbegin \DIFdel{it disappears for the }\DIFdelend \DIFaddbegin \DIFadd{the shifted line is not recorded by a spectrograph }\DIFaddend co-moving \DIFdelbegin \DIFdel{observer}\DIFdelend \DIFaddbegin \DIFadd{with the ion}\DIFaddend . At the same time, he regarded it as \textit{real}, in the sense that it would not occur if the old kinematics held\DIFaddbegin \DIFadd{, and thus is empirically testable}\DIFaddend . From the 1920s onward, Einstein argued that the behavior of atomic clocks calls for a dynamical explanation. The present paper shows, however, that even if a special relativistic explanation of the spectral identity of atoms were available, it would merely reinforce the claim that relativistic time dilation admits a purely kinematical interpretation. \DIFaddbegin \DIFadd{This point also clarifies the relation between the present argument and Bell’s constructive strategy. }\DIFaddend It concludes that modern \DIFdelbegin \DIFdel{dynamists }\DIFdelend \DIFaddbegin \DIFadd{dynamicists }\DIFaddend misread Einstein’s plea for a dynamical account of clock behavior as a demand for \emph{explanation} of \DIFdelbegin \DIFdel{the }\DIFdelend time dilation, whereas it was primarily motivated by the problem of its \emph{confirmation}. 
 \end{abstract}
...
\citet{Einstein1911d} }\hskip0pt%DIFAUXCMD
}\DIFaddend felt the need to take a public stance against this view. Length contraction is indeed \emph{apparent} insofar as it is \DIFdelbegin \DIFdel{a coordinate effect that vanishes for a }\DIFdelend \DIFaddbegin \DIFadd{not present for observers }\DIFaddend co-moving \DIFdelbegin \DIFdel{observer, }\DIFdelend \DIFaddbegin \DIFadd{with the rod; }\DIFaddend but it is nevertheless \emph{real} \DIFdelbegin \DIFdel{, since it inevitably appears for non-co-moving observers and therefore cannot be eliminated in all frames at once. For this reason, it gives rise to physically meaningful and , }\DIFdelend \DIFaddbegin \DIFadd{in Einstein’s own operational sense, namely measurable with rods and clocks. Consider a rod $AB$ at rest in $K'$ and moving with respect to $K$. Its }\emph{\DIFadd{projection}} \DIFaddend in \DIFdelbegin \DIFdel{principle, empirically testable consequences: }%DIFDELCMD < \q{This is precisely what Ehrenfest made very clear in a very elegant way} %%%
\DIFdel{\mbox{%DIFAUXCMD
\citep[510]{Einstein1911d}}\hskip0pt%DIFAUXCMD
. As \mbox{%DIFAUXCMD
\citet{Born1909} }\hskip0pt%DIFAUXCMD
showed, in the case of linear acceleration the proper length }\DIFdelend \DIFaddbegin \DIFadd{$K$ is the segment $A'B'$ determined by the simultaneous positions $A'$ and $B'$, in $K$, }\DIFaddend of \DIFdelbegin \DIFdel{a rod can, in principle, be preserved by appropriately coordinating the accelerations of its different parts. In the case of a rotating disk, by contrast, no global }\DIFdelend \DIFaddbegin \DIFadd{the endpoints $A$ and $B$. The segment $A'B'$ is shorter than the proper length $AB$ of the rod as measured in its rest frame. In principle, this prediction can be confirmed or disconfirmed by measuring $AB$ and $A'B'$ with the same unit rod that is momentarily }\DIFaddend co-moving \DIFdelbegin \DIFdel{inertial frame exists, so Lorentz contractions along the rim cannot be eliminated simultaneously. The resulting mismatch between circumference and radius gives rise to internal stresses, illustrating how }%DIFDELCMD < \s{apparent} %%%
...
\DIFdelend \DIFaddbegin \DIFadd{with it and then brought to rest without alteration of its length. As Einstein put it to Varićak: }\q{The contraction can be ascertained by measurement, \ie, it is \s{real}{}}[Die Verkürzung ist konstatierbar durch Messung, also wirklich] \lettercpaep{Einstein}{\Var}{3}{3}{1911}[5{[}10{]}][257a]\DIFadd{. Needless to say, such a direct test of length contraction was not practically feasible at the time}\DIFaddend .
...
Referring to his earlier 1905 paper, Einstein reminds his readers that\DIFaddbegin \DIFadd{, }\DIFaddend in relativity theory\DIFdelbegin \DIFdel{a uniformly moving clock runs more slowly, when judged from a }\DIFdelend \DIFaddbegin \DIFadd{, successive ticks of a clock at rest in $K'$ are assigned a longer coordinate-time interval by the synchronized clocks at rest in a }\DIFaddend \s{stationary} system \DIFdelbegin \DIFdel{, than when judged }\DIFdelend \DIFaddbegin \DIFadd{$K$ than }\DIFaddend by an observer co-moving with the clock. Einstein appears implicitly to assume that any periodic system passing through identical phases can be regarded as a clock. If $\nu_0$ denotes the frequency defined as the number of periods $N$ divided by the time interval $\Delta t'$ measured in the co-moving system $K'$, and $\nu$ denotes the frequency defined as the same number $N$ divided by the coordinate time interval $\Delta t$ measured in a system $K$ relative to which the atom is moving, then, since $\Delta t=\gamma\,\Delta t'$, special relativity predicts:
...
%DIF > \item \emph{Physical premise (stability of the proper frequency).}
%DIF > Uniform translational motion does not alter the intrinsic oscillatory process responsible for emission. Hence identical ions, when compared at rest, possess the same proper frequency \(\nu_0=N/\Delta t'\).
\DIFaddbegin 

%DIF > The integer \(N\) simply counts the number of completed oscillations in the emission process and is therefore not itself a relativistic assumption. The nontrivial assumption is that uniform motion does not alter the proper period of the emitter, so that the same kind of ion is characterized by the same proper frequency \(\nu_0=N/\Delta t'\) when compared at rest.

\DIFaddend \item \emph{Experimental restriction (transverse configuration).}  
...
\DIFdel{is entirely a consequence of the relativistic transformation of time, that is, of time dilation}\DIFdelend \DIFaddbegin \DIFadd{The same emission process involves the same count \(N\) of completed oscillations. Since uniform motion is assumed not to alter the emitter’s proper frequency \(\nu_0=N/\Delta t'\), the transverse shift is not due to a change in the emitter, but to the fact that the same \(N\) is referred to different time intervals in the two inertial descriptions}\DIFaddend :
\[
\Delta t = \gamma\,\Delta t'.
\]
\end{enumerate}
...
It is well known that, beginning in the 1920s, Einstein increasingly acknowledged that a dynamical explanation of the periodic processes used as clocks would ultimately be required \citepp{Einstein1921}{Einstein1923}{Einstein1924}{Einstein1926}{Einstein1949}. From a more fundamental standpoint, clocks may be regarded as particular solutions of the underlying dynamical equations, with their behavior fixed by invariant parameters of the theory. Such parameters determine characteristic frequencies\footnote{For example, a mass parameter fixes the Compton frequency $\nu_C=\hfrac{mc^2}{h}$.}, which in turn set the intrinsic periodicity of the admissible solutions. If the fundamental theory is Lorentz invariant, the same periodic solutions must occur in all co-moving systems, with the \emph{same} proper frequency $\nu_0$, whether atomic or otherwise. In the case of atoms, such a theory would thus provide a dynamical explanation of \DIFaddbegin \DIFadd{the }\DIFaddend spectral identity of atoms---the empirical fact that identical atoms, when compared at relative rest, possess the same intrinsic frequency $\nu_0 = N/\Delta t'$ and emit the same spectral lines when measured by a spectrograph co--moving with them in the frame $K'$.

%DIF > once a genuine special-relativistic dynamics of clocks is in place, it explains why suitable clocks possess stable proper frequencies and why identical clocks remain identical when compared at rest. What it does not add, unless one moves toward a Lorentzian or Ives-style interpretation, is a further dynamical alteration of the moving clock that explains the transverse Doppler factor independently of the Lorentz transformation.
\DIFaddbegin 

\DIFaddend If such a theory were available, premises~1 and~2 would no longer be merely assumed but would be derived as consequences of the theory itself. Since premise~3 is merely definitional, there would then be no way to avoid conclusion~5, namely that prediction~4, the \tDe, is genuinely an \emph{apparent} coordinate effect. Thus, somewhat paradoxically, once a genuine dynamical explanation of the behavior of clocks is in place, \DIFdelbegin \DIFdel{one is forced to admit that clock retardation admits only a purely kinematical explanation}\DIFdelend \DIFaddbegin \DIFadd{kinematics alone explains the frequency shift across inertial frames}\DIFaddend : the \tDe appears because $\Delta t = \gamma \Delta t'$ in the non-co-moving system $K$. To avoid this conclusion one could deny premises~1 and~2 and search for dynamical explanation of the \emph{real} frequency contraction of the source. Somewhat ironically, this was precisely the strategy defended by Herbert E. Ives \citep{Ives1938} in connection with the experimental confirmation of the \tDe in the late 1930s \citep{Lalli2013}. However, Ives’ neo-Lorentzian move does not amount to a dynamical account that would \emph{explain} relativistic kinematics in the sense advocated by modern dynamicists \citep[vii]{Brown2005}; rather, it amounts to an ether-based dynamical \DIFdelbegin \DIFdel{conspiracy designed to }\emph{\DIFdel{conceal}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{reinterpretation designed to preserve }\DIFaddend an underlying classical kinematics. At this point, however, it becomes unclear what a genuinely dynamical explanation of time dilation would amount to.
...
\DIFaddbegin \DIFadd{One might object, following \mbox{%DIFAUXCMD
\citet{Bell1976}}\hskip0pt%DIFAUXCMD
, that detailed model calculations can show how Lorentz-contracted or time-dilated behavior follows from the dynamical laws governing a material system described in a single inertial frame. This is certainly correct, but it leaves open how such calculations are to be interpreted. }\begin{inparaenum}[(a)] \item \DIFadd{Either they are read in a neo-Lorentzian sense, close to Ives’s strategy, as showing that moving atoms or clocks are dynamically modified relative to a privileged state of rest; in that case time dilation is indeed a dynamical effect, but at the price of abandoning Einsteinian special relativity. }\item \DIFadd{Or they are read within special relativity itself; in that case the moving atom is not dynamically different from an atom at rest, but is the Lorentz-transformed counterpart of the same kind of physical solution. }\end{inparaenum} \DIFadd{Bell-style calculations then show how concrete systems dynamically realize Lorentz-covariant behavior, not how time dilation is replaced by an independent dynamical alteration of the clock. It is difficult to identify a third option corresponding to a distinctively dynamical interpretation of special relativity.
}\DIFaddend 

%DIF > I do not see a third option corresponding to a distinctively dynamical interpretation of special relativity.  
\DIFaddbegin 

\DIFaddend In my view, the historically motivated case for the dynamical approach ultimately rests on a misunderstanding. %
%
